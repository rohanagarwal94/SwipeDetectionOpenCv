...
bool firtTime=true;
int iSinceKFInit;
string folder = "/home/rohanagarwal94/Downloads/images/img_";
string suffix = ".jpg";
...
void waitForPalmCover(MyImage* m){
	...
	center= Point(m->src.cols/2, m->src.rows/2);
	for(int i =0;i<50;i++){
    	m->cap >> m->src;
		flip(m->src,m->src,1);
...
void initWindows(MyImage m){
    ...
    cvSetWindowProperty("img1", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
}
...
void myDrawContours(MyImage *m,HandGesture *hg){
	...
	//the red rectangle being drawn
        
	rectangle(m->src,hg->bRect.tl(),hg->bRect.br(),Scalar(0,0,200));
	
	circle( m->src , (hg->bRect.tl()+hg->bRect.br())* 0.5,   9, Scalar(0,205,0), 5 );
...
*/
		...
		//for the smaller circle for each finger
